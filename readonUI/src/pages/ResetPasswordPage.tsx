...
const ResetPasswordPage = () =>{
    ...
    useEffect(() => {
        ...
        var user = localStorage.getItem("resetUser");
        if (!user){
        ...
        if (password == cpassword){
            try{
                var response = await axios.put(
                    `https://localhost:7182/api/Auth/reset-password?username=${resetUser}&resetToken=${encodeURIComponent(resetToken)}&newPassword=${password}`,
                    null,
                    {
                    headers: {
                        Accept: "*/*",
            ...
            setErr("New Password and Confirm Password are different.");
        }
    }
    ...
    return (
        <div className="animate-fade-in flex items-center justify-center h-screen bg-[#dFDFDF]">
            <div className="flex items-center justify-center bg-white">
                <div className=" rounded-r-[32px] text-white bg-black flex flex-col items-center w-[469px] h-[575px]">
                    ...
                    <p className="font-[300] text-[60px] mt-[-8px]">ReadOn</p>
                    <p className="text-[20px] mt-[-16px]">LIBRARY</p>

                    <p className="font-[300] mt-[72px] text-center text-[21px] w-[250px]">"Your premier digital library for borrowing and reading books."</p>
                </div>
                
                <div className="flex flex-col items-center w-[469px] h-[575px]">
                    <div className="absolute mt-[16px] text-right w-[469px]">
                        <a href="check_otp" className="border-1 border-r-[white] rounded-l-[12px] py-[3px] px-[16px] text-[12px] font-[500]">BACK</a>
                    </div>
                    <svg className="mt-[120px]" width="73" height="61" viewBox="0 0 73 61" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <p className="text-[36px] mt-[16px]">Reset Password</p>
                    <p className="mt-[12px]">Please enter your new password</p>

                    <form onSubmit={handleClick} className=" flex flex-col items-center justify-center ">
                        <input
                            required
                            type="password"
                            className="pl-[16px] border-1 border-black w-[280px] h-[46px] rounded-[12px] mt-[40px]"
                        ...
                        <input
                            required
                            type="password"
                            className="pl-[16px] border-1 border-black w-[280px] h-[46px] rounded-[12px] mb-[20px] mt-[12px]"
                            placeholder="Confirm Password"
                            value = {cpassword}
                            onChange={(e) => setCPassword(e.target.value)}
                        >
                        </input>
                        <button type="submit" className="text-white bg-black text-[15px] font-[600] w-[280px] h-[46px] rounded-[12px]">RESET PASSWORD</button>
                    </form>

                    {err != "" ? <div className="text-[red] text-[7.8px] w-[280px] absolute top-[66%] text-right">{err}</div>:""}
                </div>
