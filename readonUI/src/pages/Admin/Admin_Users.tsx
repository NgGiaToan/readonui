...
const Admin_Users = () =>{
    ...
    const getList = async () => {
        try {
            const response = await axiosInstance.get(`https://localhost:7182/api/Account/user-manager?n=${search}`);
    ...
    return (
        <div className="bg-[#F2F2F2] h-screen w-screen select-none">
            ...
            {/* Add Book */}
            {addPopUp && <PopUpAdd  object= {"user"} handleOut={()=>setAddPopUp(false)} handleConfirm={()=>setRender(nanoid())}></PopUpAdd>}

            {/* Update Book */}
            {updatePopUp && <PopUpUpdate object= {"user"} handleOut={()=>setUpdatePopUp(false)} id={choice} handleConfirm={()=>setRender(nanoid())}></PopUpUpdate>}

            {/* View Book */}
            {viewPopUp && <PopUpView object= {"user"} handleOut={()=>setViewPopUp(false)} id={choice}></PopUpView>}
            
            {/* Remove Book */}
            {removePopUp && <PopUpDelete object= {"user"} handleOut={()=> setRemovePopUp(false)} id={choice} handleConfirm={()=>setRender(nanoid())}></PopUpDelete>}
                ...
                <div className="p-[20px]">
                    <div className="bg-white w-full p-[20px] rounded-[20px]">
                        <div className="flex pt-[12px]">
                            ...
                            <p className="left-[1100px] absolute">Email</p>
                            <p className="left-[1400px] absolute">Username</p>
                            ...
                            <div className="overflow-y-auto max-h-[600px] min-h-[200px]">
                                { Array.isArray(list) ? list.map((item)=>(
                                    <div className="flex pl-[60px] mt-[80px] mb-[40px] items-center relative">
                                        ...
                                        <p className="w-[300px] ml-[120px]">{item.name}</p>
                                        <p className="w-[250px] ml-[60px]">{item.email}</p>
                                        <p className="w-[200px] ml-[100px]">{item.username}</p>
                                        <button className="w-[40px] ml-[75px]"
