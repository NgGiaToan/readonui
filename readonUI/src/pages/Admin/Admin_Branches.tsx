...
const Admin_Branches = () =>{
    ...
    const getList = async () => {
        try {
            const response = await axiosInstance.get(`https://localhost:7182/api/Branch/branches?n=${search}`);
    ...
    return (
        <div className="bg-[#F2F2F2] h-screen w-screen select-none">
            ...
            {/* Add Book */}
            {addPopUp && <PopUpAdd  object= {"branch"} handleOut={()=>setAddPopUp(false)} handleConfirm={()=>setRender(nanoid())}></PopUpAdd>}

            {/* Update Book */}
            {updatePopUp && <PopUpUpdate object= {"branch"} handleOut={()=>setUpdatePopUp(false)} id={choice} handleConfirm={()=>setRender(nanoid())}></PopUpUpdate>}

            {/* View Book */}
            {viewPopUp && <PopUpView object= {"branch"} handleOut={()=>setViewPopUp(false)} id={choice}></PopUpView>}
            
            {/* Remove Book */}
            {removePopUp && <PopUpDelete object= {"branch"} handleOut={()=> setRemovePopUp(false)} id={choice} handleConfirm={()=>setRender(nanoid())}></PopUpDelete>}

            <div className="pl-[222px] pt-[71px]">
            <div className="flex justify-between pt-[28px] px-[20px]">
                    <p className="text-[28px] font-[500] ml-[12px]">Branch Management</p>
                    <div className="flex items-center ">
                        ...
                        >
                            ...
                            <p className="ml-[8px]">Add Branch</p>
                ...
                <div className="p-[20px]">
                    <div className="bg-white w-full p-[20px] rounded-[20px]">
                        <div className="flex pt-[12px]">
                            <p className="left-[400px] absolute">ID</p>
                            <p className="left-[720px] absolute">Name</p>
                            <p className="left-[1100px] absolute">Contact No</p>
                            <p className="left-[1400px] absolute">Location</p>
                            <p className="left-[1700px] absolute">Action</p>
                        </div>
                        ...
                        {(!list || list.length === 0) ? (
                            ...
                            <div className="overflow-y-auto max-h-[600px] min-h-[200px]">
                                { Array.isArray(list) ? list.map((item)=>(
                                    <div className="flex pl-[60px] mt-[80px] mb-[40px] items-center relative">
                                        <p className="w-[240px]">{formatGuid(item.id)}</p>
                                        <p className="w-[300px] ml-[100px]">{item.name}</p>
                                        <p className="w-[200px] ml-[120px]">{item.contactNo}</p>
                                        <p className="w-[240px] ml-[40px]">{item.location}</p>
                                        <button className="w-[40px] ml-[110px]"
