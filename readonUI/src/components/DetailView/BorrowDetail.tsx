...
const BorrowDetail = ({id, handleOut, handleRemove}:LoanDetailProps) => {
    const [bookId, setBookId] = useState<BookId[]>([]);
    const [bookList, setBookList] = useState<BookList[]>([]);
    const [isFetching, setIsFetching] = useState(false); 

    useEffect(() => {
        if (bookId.length > 0 && !isFetching) {
            setIsFetching(true);
            getBookList();
        }
    }, [bookId]);
    ...
    const getBookList = async () => {
        bookId.map(async (item)=>{
            if (!bookList.some(book => book.id === item.bookId)) {
                try {
                    const response = await axiosInstance.get(`https://localhost:7182/api/Book/view-book?id=${item.bookId}`)
                    if (response.status = 200){
                        setBookList(prev =>
                        [
                            ...prev, 
    ...
    const handleClick = (n:string) => {
        ...
        setBookId((prev) => prev.filter(item => item.bookId !== n))
        setBookList((prev) =>prev.filter(item => item.id !== n))
    ...
    const handleConfirm = async () => {
        try {
            const response = await axiosInstance.post(`https://localhost:7182/api/Loan/user-borrewed?userId=${id}`)
                ...
                <div className="h-[729px] rounded-[16px] w-[1140px] bg-white flex flex-col justify-center items-center">
                    <div className=" flex flex-col items-center h-[558px] border-1 border-[#E3E3E3] rounded-[16px] w-[1087px]">
                        <div className="flex mt-[16px] text-[19px] font-[500]">
                            ...
                            <p className="absolute left-[720px]">Name</p>
                            <p className="absolute left-[1080px]">Type</p>
                            <p className="absolute left-[1240px]">Language</p>
                            <p className="absolute left-[1380px]">Action</p>
                        </div>
                        <div className="h-[1px] w-[1040px] mt-[36px] bg-[#E3E3E3]"></div>

                        <div className="h-[500px] overflow-y-auto w-full pt-[28px]">
                            {(!bookList || bookList.length === 0) ? (
                                <p className="text-gray-500 mt-[12px] text-[18px] ml-[40px] ">No data available</p>
                            ) :(bookList) && (bookList.map((item)=>
                                <div className="flex mb-[40px] items-center">
                                    <p className="w-[160px] ml-[60px]">{item.id}</p>
                                    <p className="w-[390px] ml-[40px]">{item.name}</p>
                        ...
                        <div className="flex items-center py-[12px]">
                            ...
                            <div>
                                    <div className="flex">
                                        ...
                                        <p className="ml-[8px] text-[16px]">{(bookId.length).toString()} Books</p>
                                    </div>
                                    <div className="flex mt-[16px]">
                                        <p className="text-[16px] font-[500]">Borrow Date:</p> 
                                        <p className="ml-[8px] text-[16px]">{(new Date()).toLocaleDateString("vi-VN")}</p>
                                    </div>
